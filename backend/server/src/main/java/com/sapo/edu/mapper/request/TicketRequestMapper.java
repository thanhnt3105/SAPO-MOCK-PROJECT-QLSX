...
public class TicketRequestMapper {
    ...
    public Ticket toNewTicket(TicketRequest ticketRequest) {
        ...
        String code;
        motorbike = motorbikeRepository.findById(ticketRequest.getMotorbikeId()).orElseThrow(() -> new EntityNotFoundException(Motorbike.class, "motorbikeId", ticketRequest.getMotorbikeId().toString()));
        if (ticketRequest.getCustomerId() != null) {
            customer = customerRepository.findById(ticketRequest.getCustomerId()).orElseThrow(() -> new EntityNotFoundException(Customer.class, "customerId", ticketRequest.getCustomerId().toString()));
        }
        if (ticketRequest.getRepairingEmployeeId() != null) {
            repairingEmployee = employeeRepository.findById(ticketRequest.getRepairingEmployeeId()).orElseThrow(() -> new EntityNotFoundException(Employee.class, "repairingEmployeeId", ticketRequest.getRepairingEmployeeId().toString()));
        }
        do {
        ...
        ticket.setPaymentMethod(ticket.getPaymentMethod());
        // not enough field for Ticket object: Set<TicketService>, Set<TicketProduct>, BigDecimal totalPrice
        return ticket;
    }
}
