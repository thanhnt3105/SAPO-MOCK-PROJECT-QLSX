...
public class EmployeeRequestMapper {
    ...
    public Employee toEmployee(EmployeeRequest employeeRequest) {
        ...
        if (strRoles != null && strRoles.stream().anyMatch(Arrays.asList("admin", "moderator", "cashier")::contains)) {
            ...
            strRoles.forEach(role -> {
                switch (role.toLowerCase()) {
                    case "admin":
                        Role adminRole = roleRepository.findByName(ERole.ROLE_ADMIN).orElseThrow(() -> new EntityNotFoundException(Role.class, "role", ERole.ROLE_ADMIN.name()));
                        roles.add(adminRole);
                        break;
                    case "cashier":
                        Role cashierRole = roleRepository.findByName(ERole.ROLE_CASHIER).orElseThrow(() -> new RuntimeException("Error: Role is not found."));
                    ...
                    // default is mechanic
                    default:
                        Role userRole = roleRepository.findByName(ERole.ROLE_MECHANIC).orElseThrow(() -> new RuntimeException("Error: Role is not found."));
                        roles.add(userRole);
                }
            });
        ...
        } else if (strRoles == null || strRoles.contains("mechanic")) {
            Role userRole = roleRepository.findByName(ERole.ROLE_MECHANIC).orElseThrow(() -> new EntityNotFoundException(Role.class, "role", ERole.ROLE_MECHANIC.name()));
            roles.add(userRole);
            employee.setRoles(roles);
        }
        do {
